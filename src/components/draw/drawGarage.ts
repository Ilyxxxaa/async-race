...
export default class DrawGarage {
  ...
  async drawCars(obj: State) {
    const state = obj;
    state.cars = [];
    const data = await this.controller.getCars(+obj.page);
    ...
    if (inputCreateCar && inputCreateColor && createCarBtn) {
      createCarBtn.addEventListener('click', () => {
        if (inputCreateCar.value && inputCreateColor.value) {
          ...
          try {
            this.controller.createCar(newCar);
          } catch (err) {
            console.log(err);
          }
          this.drawCars(obj);
  ...
  async updateCar(id: number, car: UpdateCar, obj: State) {
    await this.controller.updateCar(id, car);
    this.drawCars(obj);
  }
    ...
    startRaceBtn?.addEventListener('click', () => {
      ...
      winner.then((car) => {
        if (car.id) {
          ...
          this.controller.saveWinner(winnerInfo);
        }
        if (modalWindow) {
  ...
  async addListenerToGenerateBtn(obj: State) {
    ...
    generateBtn?.addEventListener('click', (event) => {
      ...
      Promise.all(cars.map(async (car) => this.controller.createCar(car)));
      this.drawCars(obj);
      btn.disabled = false;
    });
  }
  ...
  async startDriving(id: number, obj: State) {
    const state = obj;
    const { velocity, distance }: AnimationInfo = await this.controller.startEngine(id);
    ...
    const { success } = await this.controller.drive(id);
  ...
  async stopDriving(id: number, obj: State) {
    ...
    const carContainer = document.getElementById(`${id}`);
    const car = carContainer?.querySelector<HTMLElement>('.car-image');
    await this.controller.stopEngine(id).then(() => {
